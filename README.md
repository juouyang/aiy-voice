...
- 固定家庭背景放在裝置私有的 `~/.config/aiy-voice/assistant-profile.md`，每次請求重新讀取，最多 1,200 個字元。適合放地點、共享使用情境與家庭成員角色；不可提交到 repository。請從 [範例](examples/assistant-profile.md.example) 複製後填入裝置專屬內容。這份檔案不是模型的可寫長期記憶，模型不會自行新增或修改其中資料，也不可僅根據聲音猜測目前使用者身份。
